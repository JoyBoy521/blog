        ...
        if (imgMatch) {
            return (
                <div key={i} className="my-6 border-2 border-gray-600 bg-black p-2 group">
                    <div className="overflow-hidden relative min-h-[100px] flex items-center justify-center bg-gray-900">
                        <img 
                            src={imgMatch[2]} 
                            alt={imgMatch[1]} 
                            className="w-full h-auto opacity-90 group-hover:scale-105 transition-transform duration-500 relative z-10"
                            onError={(e) => {
                        ...
                        />
                        {/* 备用占位图 (当图片加载失败时显示) */}
                        <div className="hidden absolute inset-0 flex-col items-center justify-center text-gray-600 font-mono text-xs p-4 border border-red-900/50 bg-black/50">
                            <span className="text-red-500 font-bold mb-1">[ ! ] SIGNAL_LOST</span>
                            <span className="opacity-50 text-center break-all">{imgMatch[2]}</span>
                        </div>
                    </div>
